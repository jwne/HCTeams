package org.bson;

import org.bson.io.*;
import java.util.regex.*;
import java.util.*;
import com.mongodb.*;
import org.bson.types.*;
import java.lang.reflect.*;
import java.util.concurrent.atomic.*;
import java.nio.*;

public class BasicBSONEncoder implements BSONEncoder
{
    static final boolean DEBUG = false;
    @Deprecated
    protected OutputBuffer _buf;
    
    public byte[] encode(final BSONObject o) {
        final BasicOutputBuffer buf = new BasicOutputBuffer();
        this.set(buf);
        this.putObject(o);
        this.done();
        return buf.toByteArray();
    }
    
    public void set(final OutputBuffer out) {
        if (this._buf != null) {
            throw new IllegalStateException("in the middle of something");
        }
        this._buf = out;
    }
    
    protected OutputBuffer getOutputBuffer() {
        return this._buf;
    }
    
    public void done() {
        this._buf = null;
    }
    
    @Deprecated
    protected boolean handleSpecialObjects(final String name, final BSONObject o) {
        return false;
    }
    
    protected boolean putSpecial(final String name, final Object o) {
        return false;
    }
    
    public int putObject(final BSONObject o) {
        return this.putObject(null, o);
    }
    
    protected int putObject(final String name, final BSONObject o) {
        if (o == null) {
            throw new NullPointerException("can't save a null object");
        }
        final int start = this._buf.getPosition();
        byte myType = 3;
        if (o instanceof List) {
            myType = 4;
        }
        if (this.handleSpecialObjects(name, o)) {
            return this._buf.getPosition() - start;
        }
        if (name != null) {
            this._put(myType, name);
        }
        final int sizePos = this._buf.getPosition();
        this._buf.writeInt(0);
        List transientFields = null;
        final boolean rewriteID = myType == 3 && name == null;
        if (myType == 3) {
            if (rewriteID && o.containsField("_id")) {
                this._putObjectField("_id", o.get("_id"));
            }
            final Object temp = o.get("_transientFields");
            if (temp instanceof List) {
                transientFields = (List)temp;
            }
        }
        if (o instanceof Map) {
            for (final Map.Entry<String, Object> e : ((Map)o).entrySet()) {
                if (rewriteID && e.getKey().equals("_id")) {
                    continue;
                }
                if (transientFields != null && transientFields.contains(e.getKey())) {
                    continue;
                }
                this._putObjectField(e.getKey(), e.getValue());
            }
        }
        else {
            for (final String s : o.keySet()) {
                if (rewriteID && s.equals("_id")) {
                    continue;
                }
                if (transientFields != null && transientFields.contains(s)) {
                    continue;
                }
                final Object val = o.get(s);
                this._putObjectField(s, val);
            }
        }
        this._buf.write(0);
        this._buf.writeInt(sizePos, this._buf.getPosition() - sizePos);
        return this._buf.getPosition() - start;
    }
    
    protected void _putObjectField(final String name, Object val) {
        if (name.equals("_transientFields")) {
            return;
        }
        if (name.contains("\u0000")) {
            throw new IllegalArgumentException("Document field names can't have a NULL character. (Bad Key: '" + name + "')");
        }
        if (name.equals("$where") && val instanceof String) {
            this._put((byte)13, name);
            this._putValueString(val.toString());
            return;
        }
        val = BSON.applyEncodingHooks(val);
        if (val == null) {
            this.putNull(name);
        }
        else if (val instanceof Date) {
            this.putDate(name, (Date)val);
        }
        else if (val instanceof Number) {
            this.putNumber(name, (Number)val);
        }
        else if (val instanceof Character) {
            this.putString(name, val.toString());
        }
        else if (val instanceof String) {
            this.putString(name, val.toString());
        }
        else if (val instanceof ObjectId) {
            this.putObjectId(name, (ObjectId)val);
        }
        else if (val instanceof BSONObject) {
            this.putObject(name, (BSONObject)val);
        }
        else if (val instanceof Boolean) {
            this.putBoolean(name, (Boolean)val);
        }
        else if (val instanceof Pattern) {
            this.putPattern(name, (Pattern)val);
        }
        else if (val instanceof Map) {
            this.putMap(name, (Map)val);
        }
        else if (val instanceof Iterable) {
            this.putIterable(name, (Iterable)val);
        }
        else if (val instanceof byte[]) {
            this.putBinary(name, (byte[])val);
        }
        else if (val instanceof Binary) {
            this.putBinary(name, (Binary)val);
        }
        else if (val instanceof UUID) {
            this.putUUID(name, (UUID)val);
        }
        else if (val.getClass().isArray()) {
            this.putArray(name, val);
        }
        else if (val instanceof Symbol) {
            this.putSymbol(name, (Symbol)val);
        }
        else if (val instanceof BSONTimestamp) {
            this.putTimestamp(name, (BSONTimestamp)val);
        }
        else if (val instanceof CodeWScope) {
            this.putCodeWScope(name, (CodeWScope)val);
        }
        else if (val instanceof Code) {
            this.putCode(name, (Code)val);
        }
        else if (val instanceof DBRefBase) {
            final BSONObject temp = new BasicBSONObject();
            temp.put("$ref", ((DBRefBase)val).getRef());
            temp.put("$id", ((DBRefBase)val).getId());
            this.putObject(name, temp);
        }
        else if (val instanceof MinKey) {
            this.putMinKey(name);
        }
        else if (val instanceof MaxKey) {
            this.putMaxKey(name);
        }
        else if (!this.putSpecial(name, val)) {
            throw new IllegalArgumentException("can't serialize " + val.getClass());
        }
    }
    
    private void putArray(final String name, final Object array) {
        this._put((byte)4, name);
        final int sizePos = this._buf.getPosition();
        this._buf.writeInt(0);
        for (int size = Array.getLength(array), i = 0; i < size; ++i) {
            this._putObjectField(String.valueOf(i), Array.get(array, i));
        }
        this._buf.write(0);
        this._buf.writeInt(sizePos, this._buf.getPosition() - sizePos);
    }
    
    private void putIterable(final String name, final Iterable l) {
        this._put((byte)4, name);
        final int sizePos = this._buf.getPosition();
        this._buf.writeInt(0);
        int i = 0;
        for (final Object obj : l) {
            this._putObjectField(String.valueOf(i), obj);
            ++i;
        }
        this._buf.write(0);
        this._buf.writeInt(sizePos, this._buf.getPosition() - sizePos);
    }
    
    private void putMap(final String name, final Map m) {
        this._put((byte)3, name);
        final int sizePos = this._buf.getPosition();
        this._buf.writeInt(0);
        for (final Map.Entry entry : m.entrySet()) {
            this._putObjectField(entry.getKey().toString(), entry.getValue());
        }
        this._buf.write(0);
        this._buf.writeInt(sizePos, this._buf.getPosition() - sizePos);
    }
    
    protected void putNull(final String name) {
        this._put((byte)10, name);
    }
    
    protected void putUndefined(final String name) {
        this._put((byte)6, name);
    }
    
    protected void putTimestamp(final String name, final BSONTimestamp ts) {
        this._put((byte)17, name);
        this._buf.writeInt(ts.getInc());
        this._buf.writeInt(ts.getTime());
    }
    
    protected void putCodeWScope(final String name, final CodeWScope code) {
        this._put((byte)15, name);
        final int temp = this._buf.getPosition();
        this._buf.writeInt(0);
        this._putValueString(code.getCode());
        this.putObject(code.getScope());
        this._buf.writeInt(temp, this._buf.getPosition() - temp);
    }
    
    protected void putCode(final String name, final Code code) {
        this._put((byte)13, name);
        final int temp = this._buf.getPosition();
        this._putValueString(code.getCode());
    }
    
    protected void putBoolean(final String name, final Boolean b) {
        this._put((byte)8, name);
        this._buf.write(((boolean)b) ? 1 : 0);
    }
    
    protected void putDate(final String name, final Date date) {
        this._put((byte)9, name);
        this._buf.writeLong(date.getTime());
    }
    
    protected void putNumber(final String name, final Number n) {
        if (n instanceof Integer || n instanceof Short || n instanceof Byte || n instanceof AtomicInteger) {
            this._put((byte)16, name);
            this._buf.writeInt(n.intValue());
        }
        else if (n instanceof Long || n instanceof AtomicLong) {
            this._put((byte)18, name);
            this._buf.writeLong(n.longValue());
        }
        else {
            if (!(n instanceof Float) && !(n instanceof Double)) {
                throw new IllegalArgumentException("can't serialize " + n.getClass());
            }
            this._put((byte)1, name);
            this._buf.writeDouble(n.doubleValue());
        }
    }
    
    protected void putBinary(final String name, final byte[] data) {
        this.putBinary(name, 0, data);
    }
    
    protected void putBinary(final String name, final Binary val) {
        this.putBinary(name, val.getType(), val.getData());
    }
    
    private void putBinary(final String name, final int type, final byte[] data) {
        this._put((byte)5, name);
        int totalLen = data.length;
        if (type == 2) {
            totalLen += 4;
        }
        this._buf.writeInt(totalLen);
        this._buf.write(type);
        if (type == 2) {
            this._buf.writeInt(totalLen - 4);
        }
        final int before = this._buf.getPosition();
        this._buf.write(data);
        final int after = this._buf.getPosition();
    }
    
    protected void putUUID(final String name, final UUID val) {
        this._put((byte)5, name);
        this._buf.writeInt(16);
        this._buf.write(3);
        this._buf.writeLong(val.getMostSignificantBits());
        this._buf.writeLong(val.getLeastSignificantBits());
    }
    
    protected void putSymbol(final String name, final Symbol symbol) {
        this._putString(name, symbol.getSymbol(), (byte)14);
    }
    
    protected void putString(final String name, final String value) {
        this._putString(name, value, (byte)2);
    }
    
    private void _putString(final String name, final String s, final byte type) {
        this._put(type, name);
        this._putValueString(s);
    }
    
    protected void putObjectId(final String name, final ObjectId oid) {
        this._put((byte)7, name);
        this._buf.writeIntBE(oid._time());
        this._buf.writeIntBE(oid._machine());
        this._buf.writeIntBE(oid._inc());
    }
    
    private void putPattern(final String name, final Pattern p) {
        this._put((byte)11, name);
        this._buf.writeCString(p.pattern());
        this._buf.writeCString(BSON.regexFlags(p.flags()));
    }
    
    private void putMinKey(final String name) {
        this._put((byte)(-1), name);
    }
    
    private void putMaxKey(final String name) {
        this._put((byte)127, name);
    }
    
    @Deprecated
    protected void _put(final byte type, final String name) {
        this._buf.write(type);
        this._buf.writeCString(name);
    }
    
    @Deprecated
    protected void _putValueString(final String s) {
        this._buf.writeString(s);
    }
    
    void _reset(final Buffer b) {
        b.position(0);
        b.limit(b.capacity());
    }
    
    @Deprecated
    protected int _put(final String str) {
        return this._buf.writeCString(str);
    }
    
    @Deprecated
    public void writeInt(final int x) {
        this._buf.writeInt(x);
    }
    
    @Deprecated
    public void writeLong(final long x) {
        this._buf.writeLong(x);
    }
    
    @Deprecated
    public void writeCString(final String s) {
        this._buf.writeCString(s);
    }
}
